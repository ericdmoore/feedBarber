...
Deno.test('Given Values', async () => {
	const setImages = setPath({ path: 'images.bannerImage', value: 'json::"https://cdn.image.com"' });
	const p = await parseAndPickType({ url: 'http://example.biz', txt: jsfTXT });
	const ast = await p.parser(p.data, p.url).toAST();
...
Deno.test('Mustache Values', async () => {
	const setFavicon = setPath({ path: 'images.favicon', value: 'json::"https://cdn.something.com"' });
	const assignIconToFavicon = setPath({ path: 'images.icon', value: `{{::"{{{images.favicon}}}"` });
	const p = await parseAndPickType({ url: 'http://example.biz', txt: jsfTXT });
	const ast = await p.parser(p.data, p.url).toAST();

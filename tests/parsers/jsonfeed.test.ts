...
import { JsonFeed, JsonFeedAuthor, type RespStruct  } from '../../src/lib/parsers/jsonFeed.ts';
...
const assertProp = (
	...
	if('prop' in i){
		...
		try{
			assertEquals(actual, expected, `${pre}.${i.prop}: a: ${actual} - e:${expected}`)
		}catch(er){
			console.error(er)
...
const compareAuthors = (astAuthors: typeof ASTAuthor.TYPE[], authors: (typeof JsonFeedAuthor.TYPE | undefined)[], prefix = [] as string[] )=>{
...
const compareASTtoJSFheaders = (ast: ASTjson, kind:string, jsData: RespStruct)=>{
...
const compareASTtoJSFitems = (ast: ASTjson, jsData: RespStruct)=>{
	ast.items.forEach((astItem,i,a)=>{
		...
		assertProp({prop:'language'}, astItem, {...origItem, language:origItem.language ?? 'en-US'})
		assertProp({prop:'summary'}, astItem, origItem)

		assertProp({prop:'images.bannerImage'}, astItem, {images:{bannerImage: origItem.banner_image }})
		assertProp({prop:'images.indexImage'}, astItem, {images:{indexImage: origItem.image }})
		if(origItem.date_modified){
			assertProp({prop:'modified'}, astItem.dates, {modified: new Date(origItem.date_modified).getTime() })				
		}
		if(origItem.date_published){
			assertProp({prop:'published'}, astItem.dates, {published: new Date(origItem.date_published).getTime() })
		}
		...
		astItem.attachments.forEach((attached, j)=>{
			...
			assertEquals(attached.durationInSeconds, ogAttach?.duration_in_seconds, 'attachment.durationSecs')
			assertEquals(attached.mimeType, ogAttach?.mime_type,'attachment.mime')
			assertEquals(attached.sizeInBytes, ogAttach?.size_in_bytes,'attachment.bytes')
...
Deno.test( 'DFB header Values', async () => {
		...
		const ast = await computableToJson(JsonFeed<RespStruct>(c1.data, fakeUrl).toAST());
...
Deno.test( 'HyperCritical Feed Headers', async()=>{ 
		...
		const jsData = c1.data as RespStruct
		const ast = await computableToJson(JsonFeed<RespStruct>(c1.data, url).toAST());
		compareASTtoJSFheaders(ast, c1.kind, jsData);
	}
)

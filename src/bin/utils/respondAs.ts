...
export const respondAs = async (
  ...
  if (outpuFormat === "rss") {
    const compactRssData = await rss.Rss({} as rss.RespStruct, input.url)
      .fromAST(input.ast);
    ...
    const compactAtomData = await atom.Atom({} as atom.RespStruct, input.url)
      .fromAST(input.ast);
    ...
    resp = json(ast, {
      headers: { "Content-Type": "application/json+cityfeed" },
    });
  } //
  // add else if here
  //
    ...
    const jsonDatam = await jsonfeed.JsonFeed(

...
export const awsV4Sig = (cfg: Credentials & {region: string, service: string })=> {
  return async (request: Request): Promise<Request> => {
    ...
    headers.set("Authorization", `${ALGO} Credential=${awsAccessKeyId}/${credentialScope}, SignedHeaders=${signedHeaders}, Signature=${signature}`);

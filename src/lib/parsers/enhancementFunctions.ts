...
export const functionsStruct = (() => {
  ...
  const jsonEnc = async (obj: unknown) =>
    new Uint8Array(await nodeBuffer.from(JSON.stringify(obj), "utf8").buffer);
  ...
  const jsonDec = async (data: Uint8Array) =>
    JSON.parse(await dec.decode(data));
...
export const functionsEncodings = (() => {
  const B64toURL = async (data: PromiseOr<Uint8Array>) =>
    nodeBuffer.from(await data).toString("base64").replaceAll("/", "_");
  const B64fromURL = async (URLstring: string) =>
    new Uint8Array(
      nodeBuffer.from(await URLstring.replaceAll("_", "/"), "base64").buffer,
    );

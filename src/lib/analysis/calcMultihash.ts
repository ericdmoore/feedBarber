...
const sha256 = multi.hasher.from({
  name: 'sha2-256', code: 0x12, encode: sha('SHA-256')
})

const sha512 = multi.hasher.from({
  name: 'sha2-512', code: 0x13, encode: sha('SHA-512')
})
...
export const cidHash = async (i: string | Record<string, unknown>, hahser : 'sha256' | 'sha512' = 'sha256')=>{

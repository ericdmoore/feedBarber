...
import { type ASTjson, type ASTcomputable, type ASTFeedItemJson, computableToJson, rezVal} from '../parsers/ast.ts'
import { superstruct as s } from '../../mod.ts'
import { readToString } from '../utils/pumpReader.ts'
import {
...
// import { Static, Type as t} from 'https://deno.land/x/typebox@0.23.4/src/typebox.ts'

type AST = ASTjson | ASTcomputable

// import * as polly from 'https://deno.land/x/aws_sdk@v3.32.0-1/client-polly/mod.ts'
// import { IAMClient, SimulateCustomPolicyCommand, SimulateCustomPolicyCommandInput, SimulateCustomPolicyCommandOutput } from "https://esm.sh/@aws-sdk/client-iam";
 ...
 * @param ast 
 */
export const textToVoice = (userParams: s.Infer<typeof text2VoiceParams>, pc?: PollyClientInterface, s3c?: S3Bucket) => async (_ast: PromiseOr<AST>): Promise<ASTjson> => {
    ...
    if(err){
        return Promise.reject({err:'Error Code', code: 400})
    }
    ...
    for(const item of items){
        const attachedItem = await handleItem(
            item , 
            i, 
            items)
        items[i] = attachedItem
        i++
    }
...
const placeholderURL = (status: 'inprogress'|'scheduled', x=0, y=15):string => 
    status === 'inprogress' 
        ? inProgressPlaceholderURL('in progress',x,y) 
        : scheduledPlaceholderURL('scheduled',x ,y)
...
export const haveEverStarted = async (
    ...
    if(dynC){
        ...
        return s3cache ? JSON.parse(await readToString(s3cache.body)) : null as DoubleNestedDict | null
    }
}
...
export const cacheOurBreadcrumbs = async (item: typeof ASTFeedItemJson.TYPE, itemKey:string, taskOutput: SpeechSynthesisTaskResponse, s3c: S3Bucket, dynC?: DynamoDBClient )=>{
...
export const makeItemHandler = ( config: s.Infer<typeof defCfgType>, pc: PollyClientInterface, s3c: S3Bucket, dynC?: DynamoDBClient) => 
    async (item: typeof ASTFeedItemJson.TYPE, _itemNumber:number, _list: typeof ASTFeedItemJson.TYPE[] ) : Promise<typeof ASTFeedItemJson.TYPE> => {
    ...
    if(started) {
        ...
        const cache = await cacheOurBreadcrumbs(item, k, commandResponse, s3c, dynC)

...
export const splitSynthTaskResponse = (
	...
	const { CreationTime, RequestCharacters, OutputUri, TaskId, TaskStatus, TaskStatusReason, SnsTopicArn, ...config } = resp;
	return {
		taskIDs: { CreationTime, RequestCharacters, OutputUri, TaskId, TaskStatus, TaskStatusReason, SnsTopicArn },
		config,
	};
};
...
export const cacheOurBreadcrumbs = async (
	...
	const saved = {
		...
		imageUrls: {
			...
			svgText: `data:image/svg+xml;${encodeURIComponent(placeholderURL(taskIDs.TaskStatus)(icon.rectStr, icon.fill))}`,
		},
	} as BreadcrumbCache;
...
const oneOrMoreCloudParams = {
	...
	schema: {
		anyOf: [
			{ type: jSchema.TypeName.Object, required: ['aws'], properties: { aws: cloudParams_AWS.schema } },
			{ type: jSchema.TypeName.Object, required: ['azure'], properties: { azure: cloudParams_AZURE.schema } },
			{ type: jSchema.TypeName.Object, required: ['gcloud'], properties: { gcloud: cloudParams_GCLOUD.schema } },
		],
	},
};
